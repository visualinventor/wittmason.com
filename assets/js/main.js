...
;(function(window) {
	...
	Grid.prototype.options = {
		// The default paths (sequence of paths) that all the grid items are going to use to either get hidden/covered or shown/uncovered.
		paths : {
			...
			nextStep = function(pos) {
				if( pos > stepsTotal - 1 ) {
					if( callback && typeof callback == 'function' ) {
						...
						// If the type is video then play or pause
						if( self.type === 'video' ) {
							if( action === 'uncover' ) {
	...
	GridSlideshow.prototype._navigate = function(direction) {
      ...
      //console.log('page' + (this.currentGridIdx + 1) );
      /* Analytics */
      ga ('send', 'pageview', ("Page " + (this.currentGridIdx + 1) ));
		}
